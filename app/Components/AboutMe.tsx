import { linkGithub, linkIconGithubBlack, linkIconInstaBlack, linkIconLinkedinBlack, linkIconWppBlack, linkInsta, linkLinkedin, linkWpp } from "../Utils/links"

const AboutMe = () => {
    return (
        <div id="homepage" className="animate-hero relative min-h-[95vh] bg-black text-white flex flex-col gap-5">

            <h1 className="text-5xl lg:text-5xl text-start mt-3 font-bold pl-2 text-wrap">
                Construo sites <span className="font-extrabold text-emerald-500">modernos</span> e profissionais
            </h1>
            <h2 className="text-base text-start pl-2 font-light text-gray-300 tracking-tight">
                Prezando qualidade, estetica e desempenho.
            </h2>

            <p className="text-sm text-start pl-2">
                <span className="text-emerald-500 font-black">&mdash;&gt; Sistemas digitais.</span> <span className="font-black bg-clip-text text-transparent
                bg-gradient-to-r from-gray-500 via-gray-400 to-gray-300">Solucionando problemas.</span>
            </p>

            <img src="hero-mathen.jpg" alt=""
                className="w-1/2 sm:w-1/4 rounded-full border-2 border-emerald-600 self-center" />

            <div className="grid grid-cols-4 self-center gap-2 w-1/2 sm:w-1/4">
                <a className="" href={linkInsta} target="_blank">
                    <img src={linkIconInstaBlack} alt="" className="p-1 bg-emerald-300 rounded-md" />
                </a>

                <a className="" href={linkLinkedin} target="_blank">
                    <img src={linkIconLinkedinBlack} alt="" className="bg-emerald-300 rounded-md" />
                </a>

                <a className="" href={linkWpp} target="_blank">
                    <img src={linkIconWppBlack} alt="" className="bg-emerald-300 rounded-md" />
                </a>

                <a className="" href={linkGithub} target="_blank">
                    <img src={linkIconGithubBlack} alt="" className="bg-emerald-300 rounded-md" />
                </a>
            </div>

            <p className="text-xs text-center font-bold w-10/12 self-center">
                Landing Pages. Portfolios. Sites institucionais. Sistemas Personalizados.
            </p>

            <a href={linkWpp} target="_blank"
                className="self-center font-black border-emerald-500 border p-2 text-center w-3/4 sm:w-1/3
                border-2 rounded-4xl text-emerald-500 bg-gradient-to-r from-black via-gray-800 to-gray-700 animate-bg">
                <span className="animate-pulse">
                    Entre em contato
                </span>
            </a>

            <span className="mt-5 pt-5 h-px w-10/12 border-t border-gray-400 self-center"></span>

        </div>
    )
}

export default AboutMe